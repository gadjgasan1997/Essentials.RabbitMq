...
internal class RpcCaller : IRpcCaller
{
        ...
        where TResponse : IRpcCallResponse<TRequest>
    {
        return async () =>
        {
            ...
            var connectionNameModel = new ConnectionName(key.ConnectionName);
            var exchangeNameModel = new ExchangeName(key.ExchangeName);
            var routingKeyModel = new RoutingKey(key.RoutingKey);
            var replyToModel = new RoutingKey(key.ReplyTo);
        ...
        where TRequest : IRpcCallRequest
    {
        ...
        var message = request.CreateMessage(rpcCallOptions.ContentType);
