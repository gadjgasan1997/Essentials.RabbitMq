...
public static class EventsPublisherExtensions
{
    ...
    public static TryAsync<Unit> PublishToAmqDirectAsync<TEvent>(
        ...
        where TEvent : IEvent
    {
        var publishKey = PublishKey.Create(connectionName, Exchange.AmqDirect.Name.Value, routingKey);
    ...
    public static TryAsync<Unit> PublishToAmqFanoutAsync<TEvent>(
        ...
        where TEvent : IEvent
    {
        var publishKey = PublishKey.Create(connectionName, Exchange.AmqFanout.Name.Value);
        return publisher.PublishAsync(publishKey, @event, configure);
    }
    ...
    public static TryAsync<Unit> PublishToAmqTopicAsync<TEvent>(
        ...
        where TEvent : IEvent
    {
        var publishKey = PublishKey.Create(connectionName, Exchange.AmqTopic.Name.Value, routingKey);
        return publisher.PublishAsync(publishKey, @event, configure);
    }
